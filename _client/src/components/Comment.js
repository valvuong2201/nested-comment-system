...
const Comment = ({ id, message, user, createdAt, likeCount, likedByMe }) => {
    ...
    function onToggleCommentLike() {
        return toggleLikeCommentFn.execute({ id, postId: post.id }).then((c) => {
            toggleLocalCommentLike(id, c.addLike)
        })
    }
    ...
    return (
        <>
            <div className='comment'>
                ...
                {isEditing ? <CommentForm autFocus initialValue={message} onSubmit={onCommentUpdate} loading={updateCommentFn.loading} error={updateCommentFn.error} /> : <div className='message'>{message}</div>}
                <div className='footer'>
                    <IconBtn onClick={onToggleCommentLike} disabled={toggleLikeCommentFn.loading} Icon={likedByMe ? FaHeart : FaRegHeart} aria-label={likedByMe ? 'Unlike' : 'Like'}>
                        {likeCount}
                    </IconBtn>
                    ...
                    {user.id === currentUser.id && (
                        <>
                            ...
                            <IconBtn onClick={onCommentDelete} disabled={deleteCommentFn.loading} Icon={FaTrash} aria-label='Delete' color='danger' />
                        </>
                    )}
                </div>
            ...
            </div>
            {
                isReplying && (
                    <div className='mt-1 ml-3'>
                        <CommentForm autFocus onSubmit={onCommentReply} loading={createCommentFn.loading} error={createCommentFn.error} />
                    </div>
                )
            }
